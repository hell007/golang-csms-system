...
export default{
  ...
  methods: {
		...
		isInCanvas(obj) {
			let self = this
			return obj.clientX >= self.left && obj.clientY >= self.top && obj.clientX <= (self.left + self.width) && obj.clientY <= (self.top + self.height)
		}
  },
  created() {
		let self = this
		...
		drag.$on("moveInCanvas", function (obj) {
			if (self.isInCanvas(obj)) {
				...
				if (self.domArr.length <= 0) {
					//console.log('domArr.length <=0')
					self.inCanvas = 0;
					self.tabIndex = null;
				} else if (self.domArr.length === 1) {
					//console.log('domArr.length === 1')
		...
		drag.$on("changeComponent", function (obj) {
			//console.log('changeComponent==', obj)
			for (let i = 0; i < self.components.length; i++) {
				let item = self.components[i]
				if (item.idx == obj.idx) {
